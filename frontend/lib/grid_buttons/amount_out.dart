...
class AmountOutPage extends StatefulWidget {
  const AmountOutPage({super.key});

  @override
  State<AmountOutPage> createState() => _AmountOutPageState();
}

class _AmountOutPageState extends State<AmountOutPage> {
  ...
  Future<void> _fetchBalance() async {
    ...
    if (res.statusCode == 200) {
      ...
      setState(() {
        _balance = (data['balance'] ?? 0).toDouble(); // convert int to double safely
      });
    }
  }
  ...
  Future<void> _fetchTransactions(String email) async {
    ...
    try {
      final res = await http.get(Uri.parse('http://localhost:5000/api/transactions?type=out&email=$email'));
      if (res.statusCode == 200) {
  ...
  Widget build(BuildContext context) {
    ...
    appBar: AppBar(
    title: const Text("Amount Out", style: TextStyle(color: Color(0xFF7CFC00))),
