...
class _TransactionPageState extends State<TransactionPage> {
  ...
  Future<void> _fetchTransactions(String email) async {
    ...
    try {
      final res = await http.get(Uri.parse('http://localhost:5000/api/transactions/all?email=$email'));
  ...
  void _filterTransactions() {
    setState(() {
      _filteredTransactions = _allTransactions.where((txn) {
        ...
        final afterStart = _startDate == null || date.isAfter(_startDate!.subtract(const Duration(days: 1)));
        final beforeEnd = _endDate == null || date.isBefore(_endDate!.add(const Duration(days: 1)));
    ...
    pdf.addPage(
      pw.MultiPage(
        build: (pw.Context context) => [
          ...
          pw.Text("Date Range: \${_startDate?.toString().substring(0, 10) ?? 'Any'}"
              " to \${_endDate?.toString().substring(0, 10) ?? 'Any'}"),
  ...
  Future<void> _exportToPdf() async {
    ...
    await showDialog(
      ...
      builder: (_) => AlertDialog(
        ...
        content: const Text('Would you like to save the PDF or preview it before printing?'),
        actions: [
          TextButton(
            onPressed: () async {
  ...
  Future<void> _deleteTransactionsByDateRange() async {
    ...
    final uri = Uri.parse('http://localhost:5000/api/transactions/delete-range');
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: const Text("Transactions", style: TextStyle(color: Color(0xFF7CFC00))),
            ...
            Row(
              ...
              children: [
                ...
                Row(
                  children: [
                    ElevatedButton.icon(
                      onPressed: _filteredTransactions.isEmpty ? null : _exportToPdf,
                    ...
                    ElevatedButton.icon(
                      onPressed: (_startDate == null || _endDate == null || _filteredTransactions.isEmpty)
                          ? null
                          : _confirmDelete,
                  ...
                  child: ListTile(
                    title: Text(
                      ...
                      style: const TextStyle(
                          color: Color(0xFF7CFC00), fontWeight: FontWeight.bold),
                    ),
                    subtitle: Text(
                    ...
                    trailing: Text(
                      _filteredTransactions[i]['date']?.toString()?.substring(0, 10) ?? '',
