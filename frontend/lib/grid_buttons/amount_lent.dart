...
class _AmountLentPageState extends State<AmountLentPage> {
  ...
  Future<void> _fetchTransactions(String email) async {
    ...
    try {
      final res = await http.get(
        Uri.parse('http://localhost:5000/api/transactions?type=lend&email=$email'),
      );
      if (res.statusCode == 200) {
  ...
  Future<void> _clearTransaction(String transactionId, num amount) async {
    ...
    if (res.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Transaction cleared")),
      );
      if (_email != null) _fetchTransactions(_email!);
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
        SnackBar(content: Text("Clear failed: ${res.body}")),
      );
    }
  }
          ...
          TextButton(
            onPressed: () async {
              ...
              if (entered > currentAmount) {
                ScaffoldMessenger.of(context).showSnackBar(
                  SnackBar(content: Text("Cannot clear more than ₹$currentAmount")),
                );
                return;
              }
  ...
  Future<void> _partialClearTransaction({
    ...
    final uri = Uri.parse('http://localhost:5000/api/transactions/clear-partial');
    final body = jsonEncode({
      'transactionId': transactionId,
    ...
    if (res.statusCode == 200) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text("Partial amount cleared")),
      );
      if (_email != null) _fetchTransactions(_email!);
    } else {
      ScaffoldMessenger.of(context).showSnackBar(
  ...
  Widget build(BuildContext context) {
    ...
    child: Scaffold(
      ...
      appBar: AppBar(
    title: const Text("Amount Lent", style: TextStyle(color: Color(0xFF7CFC00))),
                    ...
                    trailing: Row(
                      ...
                      children: [
                        ...
                        PopupMenuButton<String>(
                          onSelected: (value) {
                            if (value == 'clear') {
                              _clearTransaction(_transactions[i]['_id'], _transactions[i]['amount']);
                            } else if (value == 'partial') {
                              _showPartialClearDialog(
                                transactionId: _transactions[i]['_id'],
                                currentAmount: _transactions[i]['amount'],
                                currentNote: _transactions[i]['notes'],
                              );
                            }
                          },
                          itemBuilder: (context) => [
                            const PopupMenuItem(value: 'clear', child: Text('Clear')),
                            const PopupMenuItem(value: 'partial', child: Text('Partial Clear')),
                          ],
